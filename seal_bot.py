...
def check_levelup(sid):
    ...
    while True:
        ...
        if len(seal) < 11:
            ...
            break
        lvl = _safe_int(seal[9])
        exp = _safe_int(seal[10])
        ...
        if exp >= needed:
            ...
            new_max_hp = seal[4] + hp_inc
            update_seal(
                sid,
                level=nl,
                exp=ne,
                strength=seal[7] + str_inc,
                defense=seal[8] + def_inc,
...
def get_effective_stats(sid):
    ...
    if not seal:
        return 0, 0, 0
    if len(seal) < 18:
        print(f"⚠️ Ошибка: Недостаточно данных для тюленя {sid}.")
        return 0, 0, 0
    base_strength = _safe_int(seal[7])
    base_defense = _safe_int(seal[8])
    base_max_hp = _safe_int(seal[4])
...
def get_faction_disc(uid):
    p = get_player(uid)
    if not p or len(p) < 7 or not p[5]:
        return 0.0
    rep = _safe_int(p[6])
    if rep >= 100:
        return 0.30
...
def get_seal_status(seal, married):
    if not seal:
        return "❓"
    if _safe_int(seal[11]) == 1:
        return "🍼"
    if _safe_int(seal[3]) <= 0:
        return "💀"
    mood = _safe_int(seal[5])
    satiety = _safe_int(seal[6])
    if mood < 30 or satiety < 20:
        return "😴"
    if seal[0] in married:
        return "❤️"
    wc = seal[18]
    if wc:
        try:
...
def update_quest_progress(uid, qt, amt=1):
    ...
    for qid, prog, targ in c.fetchall():
        if prog < targ:
            c.execute("UPDATE daily_quests SET quest_progress=? WHERE quest_id=?", (min(targ, prog + amt), qid))
    conn.commit()
    conn.close()
...
def cmd_profile(message):
    ...
    p = get_player(uid)
    if not p:
        bot.send_message(uid, "Напишите /start")
        return
    cnt = get_seal_count(uid)
    fn = p[4]
    ...
    if seals:
        ...
        for s in seals:
            ...
            t += f"  🍖 Сытость: {s[6]}\n"
            if skills:
                t += f"  ✨ Навыки: {', '.join(sk['name'] for sk in skills)}\n"
    bot.send_message(uid, t, parse_mode='Markdown')
...
def cmd_gallery(message):
    ...
    seals = get_player_seals(uid)
    if not seals:
        bot.send_message(uid, "Нет тюленей! /start")
        return
...
def cmd_inventory(message):
    ...
    inv = get_inv(uid)
    if not inv:
        bot.send_message(uid, "Инвентарь пуст!")
        return
    t = "🎒 *Инвентарь*\n\n"
    cats = {"food": "🍴 Еда", "medkit": "💊 Медицина", "weapon": "⚔️ Оружие",
            "armor": "🛡️ Броня", "helmet": "🪖 Шлемы", "shield": "🛡️ Щиты",
            "accessory": "🎀 Аксессуары", "resource": "📦 Ресурсы"}
    grouped = {}
    for item in inv:
...
def cmd_setphoto(message):
    ...
    seals = get_player_seals(uid)
    if not seals:
        bot.send_message(uid, "Нет тюленей!")
        return
    m = types.InlineKeyboardMarkup()
    for s in seals:
        m.add(types.InlineKeyboardButton(f"{s[2]} (ур.{s[9]})", callback_data=f"sphoto_{s[0]}"))
...
def proc_seal_photo(message, sid):
    ...
    if not message.photo:
        bot.send_message(uid, "Не фото!")
        return
    try:
        fi = bot.get_file(message.photo[-1].file_id)
        dl = bot.download_file(fi.file_path)
        os.makedirs("photos", exist_ok=True)
        p = f"photos/seal_{sid}.jpg"
        with open(p, 'wb') as f:
            f.write(dl)
        update_seal(sid, photo_path=p)
        bot.send_message(uid, "✅ Фото обновлено!")
...
def menu_seal(message):
    ...
    m = types.InlineKeyboardMarkup()
    for s in seals:
        m.add(types.InlineKeyboardButton(f"{s[2]} (ур.{s[9]}){' 🍼' if s[11] == 1 else ''}", callback_data=f"sinfo_{s[0]}"))
    bot.send_message(uid, "Выберите тюленя:", reply_markup=m)
...
def seal_feed(call):
    ...
    if not food:
        bot.answer_callback_query(call.id, "Нет еды!")
        return
    m = types.InlineKeyboardMarkup()
    for i in food:
...
def seal_heal(call):
    ...
    if not seal:
        return
    if seal[3] >= seal[4]:
        bot.answer_callback_query(call.id, "Здоров!")
        return
    if get_item_qty(uid, "Аптечка 💊") <= 0:
        bot.answer_callback_query(call.id, "Нет аптечек!")
        return
    h = SHOP_ITEMS["Аптечка 💊"]["heal"]
    update_seal(sid, health=min(seal[4], seal[3] + h))
    remove_from_inv(uid, "Аптечка 💊")
    bot.answer_callback_query(call.id, f"💊 +{h} HP!")
    seal_selected(call, sid)
...
def seal_play(call):
    ...
    if not seal:
        return
    if seal[6] < 10:
        bot.answer_callback_query(call.id, "Голоден!")
        return
    wc = seal[19]
    if wc:
        try:
            ...
            if rem.total_seconds() > 0:
                bot.answer_callback_query(call.id, f"⏳ {int(rem.total_seconds() // 60)}м {int(rem.total_seconds() % 60)}с")
                return
        except:
            pass
...
def proc_rename(message, sid):
    ...
    if len(nn) > 20:
        bot.send_message(message.from_user.id, "Слишком длинное!")
        return
    update_seal(sid, name=nn)
    bot.send_message(message.from_user.id, f"✅ {nn}!")
...
def seal_equip_menu(call):
    ...
    if not gi:
        bot.answer_callback_query(call.id, "Нет экипировки!")
        return
    m = types.InlineKeyboardMarkup()
    for i in gi:
        m.add(types.InlineKeyboardButton(f"{i[2]} (x{i[4]})", callback_data=f"deq_{sid}_{i[2]}"))
    m.add(types.InlineKeyboardButton("◀️", callback_data=f"sinfo_{sid}"))
...
def seal_do_equip(call):
    ...
    if not seal:
        return
    sm = {"weapon": "equipped_weapon", "armor": "equipped_armor", "helmet": "equipped_helmet",
          "shield": "equipped_shield", "accessory": "equipped_accessory"}
    slot = sm.get(it)
    if not slot:
        bot.answer_callback_query(call.id, "Неизвестный тип!")
        return
    ci = {"equipped_weapon": 13, "equipped_armor": 14, "equipped_helmet": 15,
          "equipped_shield": 16, "equipped_accessory": 17}.get(slot)
    cv = seal[ci] if ci is not None else None
    if cv:
        add_to_inv(uid, cv, ITEM_TYPES.get(cv, "armor"), 1)
    update_seal(sid, **{slot: name})
    remove_from_inv(uid, name)
    pl = get_player(uid)
    if pl and pl[5] == "fashion" and it == "accessory":
...
def menu_shop(message):
    ...
    for n, i in SHOP_ITEMS.items():
        ...
        if i["type"] == "food":
            ...
            t += f"  {n} — 🐟{pr} (+{i['heal']}HP)\n"
        elif i["type"] == "accessory":
            t += f"  {n} — 🐟{pr} (+{ACCESSORY_BONUSES.get(n, 0)}😊)\n"
...
def shop_buy(call):
    ...
    if not info:
        ...
        return
    td = max(get_shop_disc(), get_faction_disc(uid))
    pr = int(info["price"] * (1 - td))
    if get_fishnets(uid) < pr:
        bot.answer_callback_query(call.id, "Не хватает 🐟!")
        return
    add_fishnets(uid, -pr)
    add_to_inv(uid, name, info["type"])
...
def craft_do(call):
    ...
    if not r:
        bot.answer_callback_query(call.id, "Не найден!")
        return
    if not can_craft(uid, r):
        bot.answer_callback_query(call.id, "Не хватает ресурсов!")
        return
    for res, amt in r["resources"].items():
        remove_from_inv(uid, res, amt)
    add_to_inv(uid, name, r["type"])
    update_quest_progress(uid, "craft", 1)
    update_quest_chain(uid, "craft_item", 1)
    pl = get_player(uid)
    if pl and pl[5] == "hunters":
        add_faction_rep(uid, 1)
...
def menu_work(message):
    ...
    if not m.keyboard:
        bot.send_message(uid, "Все малыши!")
        return
    bot.send_message(uid, "💼 Выберите тюленя:", reply_markup=m)
...
def work_sel(call):
    ...
    if not seal:
        return
    if seal[6] < 20:
        bot.answer_callback_query(call.id, "Сытость<20!")
        return
    wc = seal[18]
    if wc:
        try:
    ...
    for i, j in enumerate(JOBS):
        t += f"{i + 1}. {j['name']} — 🐟{j['reward_min']}-{j['reward_max']}, кд{j['cooldown_min']}м\n"
    m = types.InlineKeyboardMarkup()
    for i, j in enumerate(JOBS):
        m.add(types.InlineKeyboardButton(f"{j['name']} — 🐟{j['reward_min']}-{j['reward_max']}", callback_data=f"wdo_{sid}_{i}"))
...
def do_battle(call):
    ...
    bn = boss["name"]
    bhp = random.randint(60, 100) + seal[9] * 10
    bstr = random.randint(8, 15) + seal[9] * 2
    bdef = random.randint(3, 8) + seal[9]
    ...
    for rnd in range(1, 21):
        ...
        if any(s["effect"] == "dmg_reduce_10" for s in skills):
            dm = int(dm * 0.9)
        shp -= dm
        log.append(f"{bn} →{dm} ({seal[2]} {max(0, shp)}❤️)")
    ...
    if bhp <= 0:
        ...
        eg = int(random.randint(20, 40) * get_exp_mult())
        add_fishnets(uid, rw)
        update_seal(sid, exp=seal[10] + eg, mood=min(100, seal[5] + 15))
        lv = check_levelup(sid)
        log.append(f"\n🎉 *Победа!* 🐟{rw} +{eg}оп")
        if lv:
            log.append(f"📈 Ур.{lv}!")
...
def dng_start(call):
    ...
    if not seal:
        return
    if seal[3] <= 20:
        bot.answer_callback_query(call.id, "HP<20!")
        return
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    ...
    conn.commit()
    conn.close()
    dng_floor(call, sid, 1)

def dng_floor(call, sid, fl):
    seal = get_seal(sid)
    mon = get_dungeon_monster(fl)
    es, ed, eh = get_effective_stats(sid)
...
def dng_atk(call):
    ...
    if mon["hp"] <= 0:
        update_seal(sid, health=max(1, shp))
        log.append("\n✅ Повержен!")
        ...
        if fl >= 5:
            ...
            update_quest_chain(uid, "dungeon_complete", 1)
            update_quest_chain(uid, "dungeon_floor", fl)
            pl = get_player(uid)
            if pl and pl[5] == "explorers":
                add_faction_rep(uid, 3)
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
...
def cmd_fish(message):
    ...
    if fc:
        try:
            ...
            if rem.total_seconds() > 0:
                bot.send_message(uid, f"⏳ {int(rem.total_seconds() // 60)}м {int(rem.total_seconds() % 60)}с")
                return
        except:
            pass
    ...
    for o in opts:
        m.add(types.InlineKeyboardButton(o, callback_data=f"fh_{o}_{fish['name']}"))
    ...
    def timeout():
        ...
        if not fish_active.get(uid):
            return
        try:
            bot.edit_message_text(f"⏰ Время! {fish['name']} уплыл.", msg.chat.id, msg.message_id)
        except:
            pass
...
def duel_target(message, sid):
    ...
    if txt.startswith("@"):
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT user_id FROM players WHERE username=?", (txt[1:],))
        r = c.fetchone()
        conn.close()
        if not r:
...
def duel_accept(call):
    ...
    if not duel or duel[5] != 'pending':
        ...
        return
    if duel[2] != uid:
        bot.answer_callback_query(call.id, "Не вам!")
        return
    seals = get_player_seals(uid)
    if not seals:
        bot.answer_callback_query(call.id, "Нет тюленей!")
        return
    m = types.InlineKeyboardMarkup()
    for s in seals:
        if s[11] == 1:
...
def duel_seal(call):
    ...
    cseal = get_seal(csid)
    oseal = get_seal(osid)
    if not cseal or not oseal:
        bot.answer_callback_query(call.id, "Тюлень не найден!")
        return
    ces, ced, ceh = get_effective_stats(csid)
    oes, oed, oeh = get_effective_stats(osid)
    csk = get_seal_skills(csid)
    osk = get_seal_skills(osid)
    chp = ceh
    ohp = oeh
    ...
    while chp > 0 and ohp > 0:
        ...
        if rnd > 15:
            break
        cd = ces
        if any(s["effect"] == "berserk" for s in csk) and chp < ceh * 0.3:
            cd = int(cd * 1.5)
        if random.random() < sum(0.15 for s in csk if s["effect"] == "crit_15"):
            cd *= 2
            log.append("⚡ Крит!")
        ...
        if ohp <= 0:
            break
        od = oes
        if any(s["effect"] == "berserk" for s in osk) and ohp < oeh * 0.3:
            od = int(od * 1.5)
        if random.random() < sum(0.15 for s in osk if s["effect"] == "crit_15"):
            od *= 2
            log.append("⚡ Крит в ответ!")
    ...
    if ohp <= 0:
        winner_id = duel[1]
        log.append(f"\n🎉 *{cseal[2]} победил!*")
        rw = min(get_fishnets(duel[2]) // 10, 100)
        add_fishnets(duel[1], rw)
        add_fishnets(duel[2], -rw)
        log.append(f"💰 Награда: 🐟{rw}")
        update_quest_chain(duel[1], "duel_win", 1)
    elif chp <= 0:
        winner_id = duel[2]
        log.append(f"\n🎉 *{oseal[2]} победил!*")
        rw = min(get_fishnets(duel[1]) // 10, 100)
        add_fishnets(duel[2], rw)
        add_fishnets(duel[1], -rw)
        log.append(f"💰 Награда: 🐟{rw}")
    ...
    bot.edit_message_text("\n".join(log), call.message.chat.id, call.message.message_id, parse_mode='Markdown')
    try:
        bot.send_message(duel[1] if duel[1] != uid else duel[2], "\n".join(log), parse_mode='Markdown')
    except:
        pass
...
def duel_reject(call):
    ...
    try:
        bot.send_message(duel[1], "❌ Ваш вызов отклонён.")
    except:
        pass
...
def show_quest_chains(uid, chat_id, message_id=None):
    ...
    if message_id:
        try:
            bot.edit_message_text(t, chat_id, message_id, parse_mode='Markdown', reply_markup=m)
        except:
...
def tr_set(message, name):
    uid = message.from_user.id
    try:
        pr = int(message.text.strip())
    except:
        bot.send_message(uid, "Число!")
        return
    if pr < 1:
        bot.send_message(uid, ">0!")
        return
...
def trade_buy(call):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT seller_id,item_name,item_type,price,active FROM trade_offers WHERE offer_id=?", (oid,))
    r = c.fetchone()
    if not r or not r[4]:
        bot.answer_callback_query(call.id, "Не найден!")
        conn.close()
    ...
    add_fishnets(uid, -pr)
    add_fishnets(sid, pr)
    add_to_inv(uid, nm, it)
    c.execute("UPDATE trade_offers SET active=0 WHERE offer_id=?", (oid,))
    conn.commit()
    conn.close()
    bot.answer_callback_query(call.id, f"Куплено: {nm}!")
    try:
        bot.send_message(sid, f"💰 {nm} продан за 🐟{pr}!")
    except:
        pass
...
def trade_mine(call):
    ...
    for oid, nm, pr, act in offers:
        ...
        if act:
            m.add(types.InlineKeyboardButton(f"Снять {nm}", callback_data=f"trcan_{oid}"))
    m.add(types.InlineKeyboardButton("◀️ В меню", callback_data="trb"))
    bot.edit_message_text(t, call.message.chat.id, call.message.message_id, parse_mode='Markdown', reply_markup=m)
...
def trade_cancel(call):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT item_name,item_type,active FROM trade_offers WHERE offer_id=? AND seller_id=?", (oid, uid))
    r = c.fetchone()
    if not r or not r[2]:
        bot.answer_callback_query(call.id, "Не найден!")
        conn.close()
    ...
    conn.commit()
    conn.close()
    add_to_inv(uid, r[0], r[1])
    bot.answer_callback_query(call.id, f"Снято: {r[0]}!")
...
def clan_create_name(message):
    ...
    if len(name) > 30:
        bot.send_message(uid, "Слишком длинное!")
        return
...
def clan_create_emblem(message, name):
    ...
    try:
        ...
        bot.send_message(uid, "Число!")
        return
    if idx < 0 or idx >= len(CLAN_EMOJIS):
        bot.send_message(uid, "Неверный номер!")
        return
...
def clan_members(call):
    ...
    for mid in members:
        p = get_player(mid)
        if p:
            t += f"  @{p[1]} ({get_seal_count(mid)} тюленей)\n"
...
def clan_dng_start(call):
    ...
    cid = int(call.data.split("_")[1])
    clan = get_clan_by_user(uid)
    if not clan or clan[1] != uid:
        bot.answer_callback_query(call.id, "Только лидер!")
        return
    members = get_clan_members(cid)
    all_seals = []
    for mid in members:
        for s in get_player_seals(mid):
            if s[11] == 0 and s[9] >= CLAN_DUNGEON_MIN_LEVEL:
                all_seals.append(s)
    if not all_seals:
        bot.answer_callback_query(call.id, "Нет тюленей 4+ уровня!")
        return
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("DELETE FROM clan_dungeons WHERE clan_id=?", (cid,))
    ...
    for mid in members:
        for s in get_player_seals(mid):
            if s[11] == 0 and s[9] >= CLAN_DUNGEON_MIN_LEVEL:
                all_seals.append(s)
    if not all_seals:
        bot.edit_message_text("Нет доступных тюленей!", call.message.chat.id, call.message.message_id)
        return
...
def clan_dng_atk(call):
    ...
    if mon["hp"] <= 0:
        ...
        log.append("\n💀 Все тюлени пали...")
        for s in all_seals:
            update_seal(s[0], health=1, mood=max(0, s[5] - 20))
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("UPDATE clan_dungeons SET active=0 WHERE clan_id=?", (cid,))
        conn.commit()
...
def menu_faction(message):
    ...
    for fid, fd in FACTIONS.items():
        if p[5] == fid:
            m.add(types.InlineKeyboardButton(f"🔄 Сменить: {fd['name']}", callback_data=f"fj_{fid}"))
        else:
            m.add(types.InlineKeyboardButton(f"Вступить: {fd['name']}", callback_data=f"fj_{fid}"))
    bot.send_message(uid, t, parse_mode='Markdown', reply_markup=m)
...
def vote_cb(call):
    uid = call.from_user.id
    vote = "yes" if call.data == "vy" else "no"
    today = date.today().isoformat()
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    try:
        c.execute("INSERT INTO votes (user_id,event_type,vote,date) VALUES (?,?,?,?)", (uid, "daily", vote, today))
        conn.commit()
...
def menu_marry(message):
    ...
    if get_seal_count(uid) >= MAX_SEALS:
        bot.send_message(uid, f"Макс {MAX_SEALS}!")
        return
    m = types.InlineKeyboardMarkup()
    for s in seals:
        m.add(types.InlineKeyboardButton(f"{s[2]} (ур.{s[9]})", callback_data=f"msel_{s[0]}"))
    bot.send_message(uid, "Выберите тюленя:", reply_markup=m)
...
def marry_partner(message, sid1):
    ...
    if txt.startswith("@"):
        ...
        try:
            ...
            return
    if pid == uid:
        bot.send_message(uid, "С собой нельзя!")
        return
    ps = get_player_seals(pid)
    if not ps:
        bot.send_message(uid, "Нет тюленей у игрока!")
        return
    m = types.InlineKeyboardMarkup()
    for s in ps:
        m.add(types.InlineKeyboardButton(f"{s[2]} (ур.{s[9]})", callback_data=f"mdo_{sid1}_{s[0]}_{pid}"))
...
def marry_do(call):
    uid = call.from_user.id
    p = call.data.split("_")
    s1, s2, pid = int(p[1]), int(p[2]), int(p[3])
    se1, se2 = get_seal(s1), get_seal(s2)
    if not se1 or not se2:
        bot.answer_callback_query(call.id, "Не найден!")
        return
    if get_seal_count(uid) >= MAX_SEALS:
        bot.answer_callback_query(call.id, f"Макс {MAX_SEALS}!")
        return
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def show_quests(uid, chat_id, message_id=None):
    try:
        ...
        bot.send_message(chat_id, f"⚠️ Ошибка БД: {e}")
        return
    if not quests:
        bot.send_message(chat_id, "Задания не сгенерированы.")
        return
    ...
    if m.keyboard:
        ...
        if message_id:
            try:
                bot.edit_message_text(t, chat_id, message_id, parse_mode='Markdown')
            except:
...
def quest_claim(call):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT quest_id,quest_target,quest_progress,quest_reward,claimed FROM daily_quests WHERE quest_id=? AND claimed=0", (qid,))
    r = c.fetchone()
    if not r:
    ...
    bot.answer_callback_query(call.id, f"Получено 🐟{qrew}!")
    ...
    while True:
        ...
        try:
            ...
            for sid, hp, mood, sat in c.fetchall():
                nm = max(0, mood - random.randint(3, 8))
                ns = max(0, sat - random.randint(5, 10))
