...
def init_db():
    ...
    if "photo_path" not in cols:
        c.execute("ALTER TABLE seals ADD COLUMN photo_path TEXT")
...
# 17=equipped_accessory, 18=work_cooldown, 19=play_cooldown, 20=photo_path
...
ITEM_BONUSES = {
    # Оружие
    "Костяной меч 🗡️": {"str": 5},
    "Акулий клык 🦷": {"str": 8},
    "Трезубец 🔱": {"str": 12},
    "Китовый клинок 🐋": {"str": 15},
    # Броня
    "Чешуйчатая броня 🐟": {"def": 5, "hp": 10},
    "Панцирь краба 🦀": {"def": 8, "hp": 15},
    "Плетёная броня 🧵": {"def": 10, "hp": 25},
    "Кракеновый панцирь 🐙": {"def": 15, "hp": 40},
    # Шлемы
    "Шлем из ракушек 🐚": {"def": 3, "hp": 10},
    "Костяной шлем 💀": {"def": 5, "hp": 15},
    "Корона из зубов 👑": {"def": 8, "hp": 20},
    # Щиты
    "Щит из чешуи 🐠": {"def": 5},
    "Панцирный щит 🛡️": {"def": 8},
    "Щит кракена 🦑": {"def": 12},
}
...
# Объединённый справочник типов предметов (магазин + крафт + старые)
ITEM_TYPES = {}
for _name, _info in SHOP_ITEMS.items():
    ITEM_TYPES[_name] = _info["type"]
for _recipe in CRAFT_RECIPES:
    ITEM_TYPES[_recipe["name"]] = _recipe["type"]
# Совместимость со старыми предметами
ITEM_TYPES["Меч ⚔️"] = "weapon"
ITEM_TYPES["Щит 🛡️"] = "shield"
ITEM_TYPES["Шлем 🪖"] = "helmet"
ITEM_TYPES["Броня 👕"] = "armor"
...
def cmd_help(message):
    ...
    text += "  /profile — ваш профиль (количество тюленей)\n"
    ...
    text += "  /dungeon — подземелье (5 этажей, даёт ресурсы)\n"
    text += "  /work — отправить тюленя на работу\n\n"
    text += "*Другое:*\n"
    text += "  /marry — брак тюленей (максимум 5 тюленей)\n"
    text += "  /quests — ежедневные задания\n\n"
    text += "*Кулдауны:*\n"
    text += "  🎾 Игра — 15 мин\n"
    text += "  💼 Работа — 30–60 мин\n"
    text += "  ❤️ Регенерация — +25 HP в час\n\n"
    ...
    text += "  👑 Корона (+20) — 🐟200\n\n"
    text += "*Ресурсы:*\n"
    text += "  Добываются с боссов и монстров в подземелье.\n"
    text += "  Используются для крафта оружия и брони (/craft)."
...
def show_main_menu(user_id):
    ...
    markup.add(types.KeyboardButton("💍 Брак"))
...
def cmd_profile(message):
    ...
    bot.send_message(user_id, f"👤 *Ваш профиль*\n\nТюленей: {count} / {MAX_SEALS}", parse_mode='Markdown')
...
def seal_do_feed(call):
    ...
    bot.answer_callback_query(call.id,
        f"{seal[2]} съел {item_name}! Сытость +{info['satiety']}, настроение +{info.get('mood', 5)}")
...
def seal_heal(call):
    ...
    bot.answer_callback_query(call.id,
        f"💊 {seal[2]} вылечен на {heal_amount} HP! Теперь {new_hp}/{seal[4]}")
    seal_selected(call, seal_id)
...
def seal_equip_menu(call):
    ...
    for item in gear_items:
        markup.add(types.InlineKeyboardButton(f"{item[2]} (x{item[4]})",
                     callback_data=f"do_equip_{seal_id}_{item[2]}"))
    markup.add(types.InlineKeyboardButton("◀️ Назад", callback_data=f"sealinfo_{seal_id}"))
...
def menu_shop(message):
    ...
    for name, info in SHOP_ITEMS.items():
        if info["type"] == "food":
            ...
            text += f"  {name} — 🐟{info['price']} (лечит +{info['heal']} HP)\n"
        ...
        markup.add(types.InlineKeyboardButton(f"Купить {name} — 🐟{info['price']}",
...
def menu_craft(message):
    ...
    for recipe in CRAFT_RECIPES:
        ...
        bonus_text = ""
        if "str" in bonus:
            bonus_text += f"💪+{bonus['str']} "
        if "def" in bonus:
            bonus_text += f"🛡️+{bonus['def']} "
        if "hp" in bonus:
            bonus_text += f"❤️+{bonus['hp']}"
    ...
    for recipe in CRAFT_RECIPES:
        ...
        markup.add(types.InlineKeyboardButton(
            f"{can} {recipe['name']}",
            callback_data=f"craft_{recipe['name']}"))
...
def work_select_job(call):
    ...
    for i, job in enumerate(JOBS):
        markup.add(types.InlineKeyboardButton(
            f"{job['name']} — 🐟{job['reward_min']}-{job['reward_max']}",
            callback_data=f"workdo_{seal_id}_{i}"))
    markup.add(types.InlineKeyboardButton("◀️ Назад", callback_data="back_main"))
...
def dungeon_attack(call):
    ...
    if monster["hp"] <= 0:
        ...
        if floor >= 5:
            ...
            bot.edit_message_text("\n".join(log), call.message.chat.id, call.message.message_id,
                                  parse_mode='Markdown', reply_markup=markup)
...
def marry_get_partner(message, seal1_id):
    ...
    for s in partner_seals:
        markup.add(types.InlineKeyboardButton(
            f"{s[2]} (ур.{s[9]})",
            callback_data=f"marrydo_{seal1_id}_{s[0]}_{partner_id}"))
